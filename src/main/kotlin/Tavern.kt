...
fun visitTavern() {
    ...
    *patrons.map {it to 6.00}.toTypedArray())
    ...
    narrate("The item of day is the  $itemOfDay")
...
displayPatronBalances(patronGold)
    ...
    narrate ("The are still some patrons in the tavern")
    narrate (patrons.joinToString  ())
}
...
patronGold:MutableMap<String, Double>) {
    ...
    narrate("$patronName speak with $TAVERN_MASTER to place an order")
    if (itemPrice <= patronGold.getOrDefault(patronName,0.0)) {
        ...
        narrate("$TAVERN_MASTER says, \"Yoy need more coin for a $menuItemName\"")
...
private fun displayPatronBalances(patronGold: MutableMap<String, Double>) {
    narrate("$heroName intuitively knows how much money each patrons has ")
